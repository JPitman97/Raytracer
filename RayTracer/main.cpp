...
int main(int argc, char* argv[])
{
	...
	Colour grey(glm::vec4(169, 169, 169, 0));
	...
	Sphere scene_sphere2(glm::vec3(0.7, 0, 1), 1, maroon);
	Plane scene_plane(glm::vec3(0, 1 ,0), -1, grey);
	...
	for (int x = 0; x < WIDTH; x++)
	{
		for (int y = 0; y < HEIGHT; y++)
		{
			...
			else if (HEIGHT > WIDTH)
			{
				...
				xamnt = (x + 0.5) / WIDTH;
				yamnt = (((HEIGHT - y) + 0.5) / HEIGHT) / aspectRatio - (((HEIGHT - WIDTH) / static_cast<double>(WIDTH))
					/ 2);
			}
			else
			{
			...
			glm::vec3 f(camDir.x, camDir.y, camDir.z);
			glm::vec3 ff(camPitch.x * (xamnt - 0.5), camPitch.y * (xamnt - 0.5), camPitch.z * (xamnt - 0.5));
			glm::vec3 rayDir(camRoll.x * (yamnt - 0.5), camRoll.y * (yamnt - 0.5), camRoll.z * (yamnt - 0.5));
			
			rayDir = f + ff + rayDir;
			rayDir = glm::normalize(rayDir);

			Ray cam_ray(sceneCam.getPosition(), rayDir);
			...
			{
				intersections.push_back(object->findIntersection(cam_ray));
			}
